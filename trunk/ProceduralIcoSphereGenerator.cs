...
public class IcoSphereGenerator : MeshGenerator<IcoSphereGenerator>
{
	...
	public IcoSphereGenerator(float radius) : this(radius, 2)
	{
	}
	public IcoSphereGenerator() : this(1.0f, 2)
	{
	}
...
//ORIGINAL LINE: void addToTriangleBuffer(TriangleBuffer& buffer) const
	public void addToTriangleBuffer(ref TriangleBuffer buffer)
	{
		...
		float phi = .5f*(1.0f+Math.Sqrt(5.0f));
		float invnorm = 1/Math.Sqrt(phi *phi+1);
	...
	//ORIGINAL LINE: std::vector<int> faces(firstFaces, firstFaces + sizeof(firstFaces)/sizeof(*firstFaces));
		List<int> faces = new List<int>(firstFaces, firstFaces + firstFaces.Length);
		int size = 60;
		...
		for (uint i =0; i<faces.Count/3; i++)
		{
		...
		/// Step 5 : realize
		buffer.rebaseOffset();
		buffer.estimateVertexCount(vertices.Count);
		buffer.estimateIndexCount(size);
	
		for (ushort i =0; i<vertices.Count; i++)
		{
			addPoint(buffer, mRadius *vertices[i], vertices[i], new Vector2(texCoords[i].x, texCoords[i].y));
		}
