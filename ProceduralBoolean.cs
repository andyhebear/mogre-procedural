...
//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent to the STL vector 'erase' method in C#:
	...
	{
		const List<TriangleBuffer.Vertex> vec1 = mMesh1.getVertices();
		const List<int> ind1 = mMesh1.getIndices();
		const List<TriangleBuffer.Vertex> vec2 = mMesh2.getVertices();
		const List<int> ind2 = mMesh2.getIndices();
